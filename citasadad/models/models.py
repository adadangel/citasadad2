...
class citasadad(models.Model):
    _name = 'citasadad.citasadad'
	...
	try:
        	cr.execute("SELECT count(*) FROM citasadad_citasadad AS a WHERE (CAST(a.fecha_visualizacion AS varchar(20)) LIKE '%s')" % (valor))
		cont=cr.fetchone()[0]
	except:
		cont=0
	self.orden = cont+1;
